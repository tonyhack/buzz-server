...
# 副本结束
def playing_team_on_event_playing_destory(message_type, channel, channel_type, serialize):
  # 事件消息
  ...
  if playing != None and playing.get_complete() == False:
    proxy.Timer.remove(message.playing_, "playing_team_on_timer_playing")
    proxy.Timer.remove(message.playing_, "playing_team_on_timer_sync_ranking")

  team_types.PlayingManager.remove(message.playing_)

  proxy.Logging.debug("[team] playing(%d,%d) destory"\
...
# 副本时间定时器
def playing_team_on_timer_playing(id):
  # 获取副本对象
  ...
  proxy.Logging.debug("[team] playing_team_on_timer_playing expired")


# 副本排序定时器
def playing_team_on_timer_sync_ranking(id):
  return playing_team_sync_ranking(id);
