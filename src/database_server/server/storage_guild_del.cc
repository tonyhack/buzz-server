...
StorageGuildDel::StorageGuildDel() : async_request_loop_(NULL) {}
StorageGuildDel::~StorageGuildDel() {}

bool StorageGuildDel::Initialize(AsyncRequestLoop *async_request_loop) {
  if (NULL == async_request_loop) {
    LOG_ERROR("Init StorageGuildDel failed, async_request_loop is null.");
    return false;
  }
  ...
  // Set request handler.
  this->async_request_loop_->SetMessageHandler(protocol::StorageType::STORAGE_GUILD_DEL,
      boost::bind(&StorageGuildDel::Request, this, _1));

  return true;
}

void StorageGuildDel::Request(const protocol::AsyncRequestMessage &message) {
  // Get session.
  ...
  // Deserialize.
  protocol::StorageGuildDelRequest request;
  if (this->async_request_loop_->GetPacket()->Deserialize(request,
        message.message_.data(), message.message_.size()) == false) {
    LOG_ERROR("Deserialize message StorageGuildDelRequest failed.");
    this->ResponseFailed(protocol::ResponseType::ERROR_UNKNOWN, message);
    return;
  }

  // Execute procedure.
  *query << "CALL " << session->GetProcedure(protocol::StorageType::STORAGE_GUILD_DEL)
         << "("
         << request.guild_id_
         << ")";
  ...
  protocol::StorageGuildDelResponse response;
  response.__set_result_(true);
  this->OnResponse(response, protocol::ResponseType::SUCCESS, message);
}

void StorageGuildDel::ResponseFailed(protocol::ResponseType::type result,
    const protocol::AsyncRequestMessage &request) {
  this->OnResponse(protocol::StorageGuildDelResponse(), result, request);
}

void StorageGuildDel::OnResponse(const protocol::StorageGuildDelResponse &response,
