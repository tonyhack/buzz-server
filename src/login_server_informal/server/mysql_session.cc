...
bool MysqlSession::Initialize(const DatabaseInformation &database) {
  ...
  const GameDatabaseInformation *info =
  ...
  for(; pos < protocol::StorageType::STORAGE_MAX; ++pos) {
    GameDatabaseInformation::ProcedureMap::const_iterator iterator =
