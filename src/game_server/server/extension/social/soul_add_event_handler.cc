...
SoulAddEventHandler::SoulAddEventHandler() {}
SoulAddEventHandler::~SoulAddEventHandler() {}

bool SoulAddEventHandler::Initialize() {
  // Follow 增加英灵事件
  this->conn_ = coresh::CommunicatorSingleton::GetInstance().Follow(
      event::EventType::EVENT_SOUL_ADD, coresh::Communicator::GetChannelAll(),
      event::ChannelType::CHANNEL_ACTOR,
      boost::bind(&SoulAddEventHandler::OnEvent, this, _1, _2, _3, _4));

  return true;
}

void SoulAddEventHandler::Finalize() {
  coresh::CommunicatorSingleton::GetInstance().Unfollow(this->conn_);
}

void SoulAddEventHandler::OnEvent(core::uint64 channel, core::uint8 channel_type,
    const void *message, size_t size) {
  if (NULL == message || size != sizeof(event::EventSoulAdd) ||
      channel_type != event::ChannelType::CHANNEL_ACTOR) {
    LOG_ERROR("Invalid params.");
    return;
  }

  event::EventSoulAdd *event = (event::EventSoulAdd *)message;
