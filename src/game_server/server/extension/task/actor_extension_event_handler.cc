...
void ActorExtensionEventHandler::OnEvent(core::uint64 channel, core::uint8 channel_type,
  ...
  if(event->extension_type_ != ExtensionType::ITEM) {
     return ;
  }
  // 从玩家管理器中获取玩家对象
  ...
  // 任务玩家扩展是否存在
  TaskActor *task_actor = TaskActorManager::GetInstance()->Get(event->id_);
  if(task_actor != NULL) {
  ...
  if(database::client::DatabaseClientSingleton::GetInstance().SendRequest(
        request, database::protocol::StorageType::STORAGE_TASK_ACTOR_LOGIN,
        event->id_) == false) {
