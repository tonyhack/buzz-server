...
// Last modify: 2013-06-04 11:38:35.
...
void ActorLogoutEventHandler::OnEvent(core::uint64 channel, core::uint8 channel_type, 
  ...
  // 技能玩家对象是否已存在管理器中
  SkillRole *actor = SkillRoleManager::GetInstance()->RemoveActor(event->id_);
  if(actor == NULL) {
    global::LogError("%s:%d (%s) 玩家(%lu) 移除技能玩家对象失败",
  ...
  // 回收技能玩家对象
  actor->Finalize();
  SkillRolePool::GetInstance()->Deallocate(actor);
}
