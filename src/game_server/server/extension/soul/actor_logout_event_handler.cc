...
// Last modify: 2013-06-17 11:55:07.
...
void ActorLogoutEventHandler::OnEvent(core::uint64 channel, core::uint8 channel_type, 
  ...
  // 英灵玩家对象从管理器中删除
  SoulActor *actor = SoulActorManager::GetInstance()->Remove(event->id_);
  if(actor == NULL) {
    global::LogError("%s:%d (%s) 玩家(%lu) 移除英灵玩家对象失败",
  ...
  // 回收英灵玩家对象
  actor->Finalize();
  SoulActorPool::GetInstance()->Deallocate(actor);
}
